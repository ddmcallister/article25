...
/*
Plugin Name: dtest Email Share Form
...
function dm_html_form_code() {
        ...
        echo '<input class="ztbcampaigns-inputs" type="text" name="from-name" placeholder="Your Name" pattern="[a-zA-Z0-9_ ]+" value="' . ( isset( $_POST["from-name"] ) ? esc_attr( $_POST["from-name"] ) : '' ) . '" size="40" />';
        echo '</fieldset>';
        echo '<fieldset>';
        echo '<h3 class="ztbcampaigns-h3">';
...
function dm_deliver_mail() {
    ...
    if ( isset( $_POST['form-submitted'] ) ) {
        ...
        if (wp_mail( $to, $subject, $message, $headers)){
              ?><script>window.location.assign("http://zerotbdeaths.org/pledge-thank-you/");</script><?php exit();           
        } else {
...
add_shortcode( 'destaniemc_contact_form', 'destaniemc_shortcode');
